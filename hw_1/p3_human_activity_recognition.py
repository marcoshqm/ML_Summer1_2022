# name: Qiming Huang
# NUid: 001525526
# problem_3_Human_Activity_Recognition
...
def erm_classifier(X, valid_classes, labels):
    ...
    Sigma_hat = np.array([np.cov(X[labels == i].T) + reg for i in valid_classes])  # 11x11
...
def run_human_activity_recognition__dataset():
    print("[_Human Activity Recognition Process_]\n")
    activity_data = np.array(read_csv(script_dir + "/X_train.txt", sep='\s+'))
...
if __name__ == '__main__':
    run_human_activity_recognition__dataset()
